...
public class Proc {
	...
	public int resourceRequest(int available)
	{
		if (running)
		{
			// For simplicity we assume a process will only complete when it has acquired
			// the maximum resources it can need even though in reality a process can complete
			// without the full amount. If all resources are held, complete with some probability.
			if (heldResources == maxResources && rand.nextDouble() < 0.65)
			{
				running = false;
				int currHeld = heldResources; 
				heldResources = 0; // Relinquish all resources, reset heldResources
	...
	//clone method to make a copy when checking to see if the os will be in a safe state
	public Proc clone()
	{
		Proc copy =  new Proc(this.maxResources);
		copy.heldResources = this.heldResources;
